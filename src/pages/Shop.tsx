...
export function Shop() {
  ...
  // 计算分组购买数量
  const reduceCategoryFun = useCallback(
    index => {
      if (!menu) return "";
      let item = menu[index];
  ...
  const positionFoods = (id, index) => {
    ...
    parentNode.scrollTo({
      top: ele.offsetTop - parentNode.offsetTop,
      behavior: "smooth",
    });
  };
  ...
  return (
    <div className="shop">
      ...
      <div className="shop-info f_l">
        ...
        <div className="shop-info-manjian f_b_c">
          <div className="shop-info-manjian-item">
            {info.activity_tags &&
              info.activity_tags.map((item, index) => {
                return <span key={index}>{item.text}</span>;
              })}
          </div>
          <div className="shop-info-manjian-more">{info.activities ? info.activities.length : 0}个优惠</div>
        </div>

        <p className="shop-info-gonggao">公告：{info.promotion_info}</p>
      </div>

      <div ref={tabRef} className="shop-tab f_c_c">
        <div className={"shop-tab-item" + (tapType === 1 ? " active" : "")} onClick={() => setTapType(1)}>
          点餐
        </div>
        <div className={"shop-tab-item" + (tapType === 2 ? " active" : "")} onClick={() => setTapType(2)}>
          评价
        </div>
        <div className={"shop-tab-item" + (tapType === 3 ? " active" : "")} onClick={() => setTapType(3)}>
          商家
        </div>
      </div>
            ...
            <div className="tab1-tuijian-scroll f_l">
              <div className="tab1-tuijian-box f_l">
                {recommend.items.map(item => {
                  ...
                  let _image_path = getImageUrl(
                    image_path,
                    "x-oss-process=image/resize,m_lfit,w_240/watermark,g_se,x_4,y_4,image_YS8xYS82OGRlYzVjYTE0YjU1ZjJlZmFhYmIxMjM4Y2ZkZXBuZy5wbmc_eC1vc3MtcHJvY2Vzcz1pbWFnZS9yZXNpemUsUF8yOA%3D%3D/quality,q_90/format,webp"
                  );
            ...
            <ul>
              {menu &&
                menu.map((item, index) => {
                  ...
                  return (
                    <li
                      className={`category-menu-item ${index === menuIndex ? " active" : ""}`}
                      key={id}
                      onClick={positionFoods.bind(null, id, index)}
                    >
                      <span>{name}</span>
                      <span className="category-menu-item-number">{reduceCategoryFun(index)}</span>
                    </li>
                  );
                })}
            </ul>
                ...
                return (
                  <dl className="food-item" key={id} id={"category_" + id}>
                    ...
                    {foods.map(fitem => {
                      ...
                      let _image_path = getImageUrl(
                        image_path,
                        "x-oss-process=image/resize,m_lfit,w_140,h_140/watermark,g_se,x_4,y_4,image_YS8xYS82OGRlYzVjYTE0YjU1ZjJlZmFhYmIxMjM4Y2ZkZXBuZy5wbmc_eC1vc3MtcHJvY2Vzcz1pbWFnZS9yZXNpemUsUF8yOA%3D%3D/quality,q_90/format,webp"
                      );
                      return (
                        <dd key={item_id}>
                          <div className="food-item-food f_l">
                            ...
                            <div className="food-item-info f_l">
                              ...
                              <p>{description}</p>
                              <p>
                                月售{month_sales}份 好评率{satisfy_rate}%
                              </p>
      ...
      <div style={{ display: tapType === 2 ? "block" : "none" }}>评价</div>
      <div style={{ display: tapType === 3 ? "block" : "none" }}>商家</div>
